...
void load_weights(struct logic_gate_model &model, const std::string model_filename) {
    ...
    model.fc1_weight = ggml_new_tensor_2d(model.ctx, GGML_TYPE_F16, model.config.n_input, model.config.n_hidden);
    model.fc1_bias = ggml_new_tensor_1d(model.ctx, GGML_TYPE_F16, model.config.n_hidden);
    model.fc2_weight = ggml_new_tensor_2d(model.ctx, GGML_TYPE_F16, model.config.n_hidden, model.config.n_output);
    model.fc2_bias = ggml_new_tensor_1d(model.ctx, GGML_TYPE_F16, model.config.n_output);
    ...
    safetensors::load_from_file(model_filename, [&tensor_map](const std::string& key, const std::string& dtype, const std::vector<uint32_t>& shape, const std::vector<uint8_t>& tensor_data) {
...
void predict(struct logic_gate_model &model, std::vector<float> input) {
    ...
    struct ggml_tensor* x = ggml_new_tensor_1d(ctx, GGML_TYPE_F16, model.config.n_input);

    // 2. Define the computation graph (add 'weight' to each calculation)
    struct ggml_tensor* fc1 = ggml_add(ctx, model.fc1_bias, ggml_mul_mat(ctx, model.fc1_weight, x));  // multiply the weights, and add the bias
    struct ggml_tensor* fc1_relu = ggml_relu(ctx, fc1);
    struct ggml_tensor* fc2 = ggml_add(ctx, model.fc2_bias, ggml_mul_mat(ctx, model.fc2_weight, fc1_relu));
    ...
    ggml_backend_tensor_set(x, input_f16.data(), 0, ggml_nbytes(x));
    ...
    // print the data
    std::cout<<input[0]<<", "<<input[1]<<", got "<<ggml_fp16_to_fp32(result_data[0])<<std::endl;
