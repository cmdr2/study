...
if os.path.exists(os.path.join(config.output_dir, "unet")):
    model = UNet2DModel.from_pretrained(config.output_dir + "/unet")
    print("loaded model")
    safetensors.torch.save_file(model.state_dict(), "diff_model.sft")
else:
    model = UNet2DModel(
...
# pipe = DDPMPipeline(model, noise_scheduler).to("cuda:0")
# for i in range(1, 10):
#     x = pipe(num_inference_steps=i, generator=torch.Generator(device="cpu").manual_seed(42)).images
#     x[0].save(f"img{i}.jpg")
